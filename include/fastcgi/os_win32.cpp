...
static HANDLE g_hIoCompPort = INVALID_HANDLE_VALUE;
static HANDLE g_hStdinCompPort = INVALID_HANDLE_VALUE;
static HANDLE g_hStdinThread = INVALID_HANDLE_VALUE;

static HANDLE g_stdioHandles[3] = { INVALID_HANDLE_VALUE, INVALID_HANDLE_VALUE,
                 INVALID_HANDLE_VALUE };

// This is a nail for listening to more than one port..
static HANDLE g_acceptMutex = INVALID_HANDLE_VALUE;

static BOOLEAN g_shutdownPending = FALSE, g_shutdownNow = FALSE;
...
static FILE_TYPE g_listenType = FD_UNUSED;

// XXX This should be a DESCRIPTOR
static HANDLE g_hListen = INVALID_HANDLE_VALUE;

static BOOLEAN g_libInitialized = FALSE;
...
static int Win32NewDescriptor(FILE_TYPE type, int fd, int desiredFd)
{
    ...
    else if (fd > 0)
    {
        ...
        {
            for (int i = 1; i < WIN32_OPEN_MAX; ++i) {
...
static void StdinThread(void * startup)
{
    ...
    while (doIo) {
         ...
         */
        if (!GetQueuedCompletionStatus(g_hStdinCompPort, &bytesRead, &fd,
        ...
        if (ReadFile(g_stdioHandles[STDIN_FILENO], pOv->clientData1, bytesRead,
            &bytesRead, NULL)) {
            PostQueuedCompletionStatus(g_hIoCompPort, bytesRead,
...
int OS_LibInit(int stdioFds[3])
{
     ...
     */
    if (g_hIoCompPort == INVALID_HANDLE_VALUE) {
        g_hIoCompPort = CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, 0, 1);
        if (g_hIoCompPort == INVALID_HANDLE_VALUE) {
    ...
    if (g_acceptMutex == INVALID_HANDLE_VALUE) {
        /* If an accept mutex is in the env, use it */
        val = getenv(MUTEX_VARNAME);
        if (val != NULL) {
            g_acceptMutex = (HANDLE)atoi(val);
        }
    }
        ...
        (GetStdHandle(STD_INPUT_HANDLE) != INVALID_HANDLE_VALUE))
    {
        ...
        if (!DuplicateHandle(GetCurrentProcess(), oldStdIn,
            GetCurrentProcess(), &g_hListen,
            0, TRUE, DUPLICATE_SAME_ACCESS))
        {
            return -1;
        }

        if (!SetStdHandle(STD_INPUT_HANDLE, g_hListen)) {
            return -1;
        }
         ...
         */
        if (SetNamedPipeHandleState(g_hListen, &pipeMode, NULL, NULL))
        {
            g_listenType = FD_PIPE_SYNC;
        }
        else
        {
            g_listenType = FD_SOCKET_SYNC;
        }
    }
     ...
     */
    g_stdioHandles[STDIN_FILENO] = GetStdHandle(STD_INPUT_HANDLE);

    if (!SetHandleInformation(g_stdioHandles[STDIN_FILENO], HANDLE_FLAG_INHERIT, 0)) {
        /*
    ...
    if ((fakeFd = Win32NewDescriptor(FD_PIPE_SYNC,
        (int)g_stdioHandles[STDIN_FILENO],
        STDIN_FILENO)) == -1) {
        return -1;
    }
    else {
     ...
     */
    if (g_hStdinCompPort == INVALID_HANDLE_VALUE) {
        g_hStdinCompPort = CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, 0, 1);
        if (g_hStdinCompPort == INVALID_HANDLE_VALUE) {
    ...
    if ((cLenPtr = getenv("CONTENT_LENGTH")) != NULL &&
        atoi(cLenPtr) > 0) {
        g_hStdinThread = (HANDLE)_beginthread(StdinThread, 0, NULL);
        if (g_hStdinThread == (HANDLE)-1) {
     ...
     */
    g_stdioHandles[STDOUT_FILENO] = GetStdHandle(STD_OUTPUT_HANDLE);
    if (!SetHandleInformation(g_stdioHandles[STDOUT_FILENO],
    ...
    if ((fakeFd = Win32NewDescriptor(FD_PIPE_SYNC,
        (int)g_stdioHandles[STDOUT_FILENO],
        STDOUT_FILENO)) == -1) {
        return -1;
    }
    else {
    ...
    g_stdioHandles[STDERR_FILENO] = GetStdHandle(STD_ERROR_HANDLE);
    if (!SetHandleInformation(g_stdioHandles[STDERR_FILENO],
        HANDLE_FLAG_INHERIT, FALSE)) {
        DebugBreak();
        exit(99);
    }
    if ((fakeFd = Win32NewDescriptor(FD_PIPE_SYNC,
        (int)g_stdioHandles[STDERR_FILENO],
        STDERR_FILENO)) == -1) {
        return -1;
    }
    else {
...
void OS_LibShutdown()
{
    ...
    if (g_hIoCompPort != INVALID_HANDLE_VALUE) {
        CloseHandle(g_hIoCompPort);
        g_hIoCompPort = INVALID_HANDLE_VALUE;
    }

    if (g_hStdinCompPort != INVALID_HANDLE_VALUE) {
        CloseHandle(g_hStdinCompPort);
        g_hStdinCompPort = INVALID_HANDLE_VALUE;
    }

    if (g_acceptMutex != INVALID_HANDLE_VALUE) {
        ReleaseMutex(g_acceptMutex);
    }

    DisconnectNamedPipe(g_hListen);
...
int OS_CreateLocalIpcFd(const char *bindPath, int backlog)
{
    ...
    if (g_acceptMutex == INVALID_HANDLE_VALUE) {
        g_acceptMutex = CreateMutex(NULL, FALSE, NULL);
        if (g_acceptMutex == NULL) return -2;
        if (!SetHandleInformation(g_acceptMutex, HANDLE_FLAG_INHERIT, TRUE)) return -3;
    }
    ...
    g_listenType = (port) ? FD_SOCKET_SYNC : FD_PIPE_ASYNC;
    ...
    if (port)
    {
        ...
        pseudoFd = Win32NewDescriptor(g_listenType, listenSock, -1);
    ...
    else
    {
        ...
        pseudoFd = Win32NewDescriptor(g_listenType, (int)hListenPipe, -1);
...
int OS_FcgiConnect(char *bindPath)
{
    ...
    else
    {
        ...
        HANDLE hPipe = CreateFile(pipePath,
         ...
         * port to be used for async I/O.
         */
        if (!CreateIoCompletionPort(hPipe, g_hIoCompPort, pseudoFd, 1))
        {
    ...
    ClientData clientData)
{
    ...
    PostQueuedCompletionStatus(g_hStdinCompPort, len, STDIN_FILENO,
        (LPOVERLAPPED)pOv);
    return 0;
}
    ...
    OS_AsyncProc procPtr, ClientData clientData)
{
    ...
    if (!ReadFile(fdTable[fd].fid.fileHandle, buf, len, &bytesRead,
        ...
        if (fdTable[fd].Errno == ERROR_NO_DATA ||
            ...
            PostQueuedCompletionStatus(g_hIoCompPort, 0, fd, (LPOVERLAPPED)pOv);
            return 0;
        }
...
int OS_DoIo(struct timeval *tmo)
{
    ...
    while (ms >= 0) {
        if (tmo && (ms = tmo->tv_sec * 1000 + tmo->tv_usec / 1000) > 100)
            ms = 100;
        if (!GetQueuedCompletionStatus(g_hIoCompPort, &bytes, &fd,
...
static int isAddrOK(struct sockaddr_in * inet_sockaddr, const char * okAddrs)
{
    ...
    const char *p = strstr(okAddrs, ipaddr);
...
static int acceptNamedPipe()
{
    ...
    if (!ConnectNamedPipe(g_hListen, NULL))
    {
        switch (GetLastError())
        {
    ...
    ipcFd = Win32NewDescriptor(FD_PIPE_SYNC, (int)g_hListen, -1);
    if (ipcFd == -1) {
        DisconnectNamedPipe(g_hListen);
    }
...
#pragma warning( disable : 4127 ) 
            FD_SET((unsigned int)g_hListen, &readfds);
#pragma warning( default : 4127 ) 
...
#if NO_WSAACEPT
        hSock = accept((SOCKET)g_hListen, &sockaddr, &sockaddrLen);
        ...
        closesocket(hSock);
#else
        hSock = WSAAccept((unsigned int)g_hListen,
            &sockaddr,
            &sockaddrLen,
            isAddrOKCallback,
...
int OS_Accept(int listen_sock, int fail_on_intr, const char *webServerAddrs)
{
    ...
    if (g_acceptMutex != INVALID_HANDLE_VALUE)
    {
        if (WaitForSingleObject(g_acceptMutex, INFINITE) == WAIT_FAILED)
        {
    ...
    else if (g_listenType == FD_PIPE_SYNC)
    {
        ipcFd = acceptNamedPipe();
    }
    else if (g_listenType == FD_SOCKET_SYNC)
    {
...
void OS_SetFlags(int fd, int flags)
{
    ...
    if (fdTable[fd].type == FD_SOCKET_SYNC && flags == O_NONBLOCK) {
        if (ioctlsocket(fdTable[fd].fid.sock, FIONBIO, &pLong) == SOCKET_ERROR) {
            exit(WSAGetLastError());
        }
        if (!CreateIoCompletionPort((HANDLE)fdTable[fd].fid.sock, g_hIoCompPort, fd, 1)) {
            err = GetLastError();
            exit(err);
        }
