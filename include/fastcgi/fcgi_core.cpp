...
#define LONG_DOUBLE long double
  ...
  * Globals
  */
static int s_libInitialized = 0;
static int s_isFastCGI = -1;
static char *webServerAddressList = NULL;
static FCGX_Request s_request;
...
int FCGX_IsCGI(void)
{
    ...
    s_isFastCGI = OS_IsFcgi(FCGI_LISTENSOCK_FILENO);
...
int FCGX_OpenSocket(const char *path, int backlog)
{
    ...
    if (rc == FCGI_LISTENSOCK_FILENO && s_isFastCGI == 0) {
...
int FCGX_Init(void)
{
    ...
    FCGX_InitRequest(&s_request, FCGI_LISTENSOCK_FILENO, 0);
