...
static int s_libInitialized = FALSE;

static fd_set s_readFdSet, s_writeFdSet;
static fd_set s_readFdSetPost, s_writeFdSetPost;
static int s_numRdPosted = 0, s_numWrPosted = 0;
static int volatile g_maxFd = -1;

static int g_shutdownPending = FALSE, g_shutdownNow = FALSE;
...
int OS_LibInit(int stdioFds[3])
{
    ...
    FD_ZERO(&s_readFdSet);
    FD_ZERO(&s_writeFdSet);
    FD_ZERO(&s_readFdSetPost);
    FD_ZERO(&s_writeFdSetPost);
    ...
    ClientData clientData)
{
    ...
    FD_SET(STDIN_FILENO, &s_readFdSet);
    if (STDIN_FILENO > g_maxFd)
...
int OS_Close(int fd)
{
    ...
    if (asyncIoInUse) {
        ...
        FD_CLR(fd, &s_readFdSet);
        FD_CLR(fd, &s_readFdSetPost);
        if (asyncIoTable[index].inUse != 0) {
            asyncIoTable[index].inUse = 0;
        }

        FD_CLR(fd, &s_writeFdSet);
        FD_CLR(fd, &s_writeFdSetPost);
        index = AIO_WR_IX(fd);
        if (asyncIoTable[index].inUse != 0) {
            asyncIoTable[index].inUse = 0;
        }
...
int OS_DoIo(struct timeval *tmo)
{
    ...
    ClientData clientData;
    AioInfo *aioPtr;
    fd_set readFdSetCpy, writeFdSetCpy;

    asyncIoInUse = TRUE;
    FD_ZERO(&readFdSetCpy);
    FD_ZERO(&writeFdSetCpy);

    for (fd = 0; fd <= g_maxFd; fd++) {
        if (FD_ISSET(fd, &s_readFdSet)) {
            FD_SET(fd, &readFdSetCpy);
        }
        if (FD_ISSET(fd, &s_writeFdSet)) {
     ...
     * any work to do.
     */
    if (s_numRdPosted == 0 && s_numWrPosted == 0) {
        selectStatus = select((g_maxFd + 1), &readFdSetCpy, &writeFdSetCpy,
            NULL, tmo);
        if (selectStatus < 0) {
            exit(errno);
        }
        ...
        for (fd = 0; fd <= g_maxFd; fd++) {
            ...
            if (FD_ISSET(fd, &readFdSetCpy)) {
                s_numRdPosted++;
                FD_SET(fd, &s_readFdSetPost);
                FD_CLR(fd, &s_readFdSet);
            }

            if (FD_ISSET(fd, &writeFdSetCpy)) {
                s_numWrPosted++;
                FD_SET(fd, &s_writeFdSetPost);
                FD_CLR(fd, &s_writeFdSet);
            }
        }
    }
     ...
     */
        if (FD_ISSET(fd, &s_readFdSetPost)
     ...
     */
        if (FD_ISSET(fd, &s_writeFdSetPost) &&
...
int OS_Accept(int listen_sock, int fail_on_intr, const char *webServerAddrs)
{
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if (socket < 0) {
                if (g_shutdownPending || !is_reasonable_accept_errno(errno)) {
                    int errnoSave = errno;
