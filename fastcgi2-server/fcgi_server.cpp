...
#include "fastcgi2-ext/ThreadPool.h"
...
namespace fastcgi
{
    ...
    const Globals* FCGIServer::globals() const {
        return globals_.get();
    }

    Logger* FCGIServer::logger() const {
        return globals_->logger();
    }

    FCGIServer::Status FCGIServer::status() const {
        std::unique_lock<std::mutex> l(statusInfoMutex_);
        return status_;
    }
    ...
    void FCGIServer::start() {
        ...
        stopThread_.reset(new std::thread(std::mem_fn(&FCGIServer::stopThreadFunction), this));
    }

    void FCGIServer::stopThreadFunction() {
        while (true) {
            char c;
    ...
    void FCGIServer::initMonitorThread() {
        ...
        if (-1 == setsockopt(monitorSocket_, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one))) {
            throw std::runtime_error("Cannot reuse monitor port: " + i_cast_str<int>(errno));
        }
        ...
        monitorThread_.reset(new std::thread(std::mem_fn(&FCGIServer::monitor), this));
    }

    void FCGIServer::initRequestCache() {
    ...
    void FCGIServer::initTimeStatistics() {
    ...
    void FCGIServer::createWorkThreads() {
        for (auto i = endpoints_.begin(); i != endpoints_.end(); ++i) {
            for (unsigned short t = 0; t < (*i)->threads(); ++t) {
                globalPool_.push_back(std::thread(std::mem_fn(&FCGIServer::handle), this, i->get()));
            }
        }
    }

    void FCGIServer::initFastCGISubsystem() {
        if (0 != FCGX_Init()) {
        ...
        for (auto i = v.begin(), end = v.end(); i != end; ++i) {
            std::shared_ptr<Endpoint> endpoint(new Endpoint(
                ...
                str_cast_i<unsigned>(globals_->config()->asString(*i + "/threads"))));
    ...
    void FCGIServer::handle(Endpoint *endpoint) {
        ...
        while (true) {
            try {
                ...
                if (status < 0) {
                    throw std::runtime_error("failed to accept fastcgi request: " + i_cast_str<unsigned short>(status));
                }
    ...
    void FCGIServer::handleRequest(RequestTask task) {
    ...
    void FCGIServer::pid(const std::string &file) {
        try {
    ...
    std::string FCGIServer::getServerInfo() const {
        ...
        if (RUNNING == stat) {
            ...
            std::stringstream s;
            s << "<endpoint_pools>\n";
            for (auto i = endpoints_.cbegin(); i != endpoints_.cend(); ++i) {
                s << "<endpoint"
                    << " socket=\"" << (*i)->toString() << "\""
                    << " threads=\"" << (*i)->threads() << "\""
            ...
            for (auto i = pools.cbegin(); i != pools.cend(); ++i) {
