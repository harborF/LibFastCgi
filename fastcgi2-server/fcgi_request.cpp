...
namespace fastcgi
{
    ...
    int FastcgiRequest::read(char *buf, int size) {
        return FCGX_GetStr(buf, size, fcgiRequest_.in);
    }

    static void generateRequestInfo(const Request *request, std::stringstream &str) {
    ...
    void FastcgiRequest::write(std::streambuf *buf) {
    ...
    void FastcgiRequest::setHandlerDesc(const HandlerSet::HandlerDescription *handler) {
        handler_ = handler;
    }
