...
namespace fastcgi
{
    ...
    void Endpoint::ScopedBusyCounter::increment() {
        if (!incremented_) {
    ...
    void Endpoint::ScopedBusyCounter::decrement() {
        if (incremented_) {
    ...
    int Endpoint::socket() const {
        std::unique_lock<std::mutex> sl(mutex_);
        return socket_;
    }

    unsigned short Endpoint::threads() const {
        return threads_;
    }
    ...
    unsigned short Endpoint::getBusyCounter() const {
        std::unique_lock<std::mutex> sl(mutex_);
        return busy_count_;
    }

    void Endpoint::openSocket(const int backlog) {
        std::unique_lock<std::mutex> sl(mutex_);
    ...
    void Endpoint::incrementBusyCounter() {
        std::unique_lock<std::mutex> sl(mutex_);
        busy_count_ += 1;
    }

    void Endpoint::decrementBusyCounter() {
        std::unique_lock<std::mutex> sl(mutex_);
