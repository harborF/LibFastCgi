...
#include "fastcgi2-ext/parser.h"
#include "fastcgi2-ext/RequestCache.h"
#include "fastcgi2-ext/range.h"
#include "fastcgi2-ext/RequestImpl.h"
...
namespace fastcgi
{
    ...
    const std::string& File::remoteName() const {
        return name_;
    }

    DataBuffer File::data() const {
        return data_;
    }
    ...
    unsigned short RequestImpl::getServerPort() const {
        const std::string &res = Parser::get(vars_, SERVER_PORT_KEY);
        return (!res.empty()) ? str_cast_i<unsigned short>(res) : 80;
    }

    const std::string& RequestImpl::getHost() const {
        return Parser::get(headers_, HOST_KEY);
    }

    const std::string& RequestImpl::getServerAddr() const {
        return Parser::get(vars_, SERVER_ADDR_KEY);
    }

    const std::string& RequestImpl::getPathInfo() const {
        return Parser::get(vars_, PATH_INFO_KEY);
    }

    const std::string& RequestImpl::getPathTranslated() const {
        return Parser::get(vars_, PATH_TRANSLATED_KEY);
    }

    const std::string& RequestImpl::getScriptName() const {
        return Parser::get(vars_, SCRIPT_NAME_KEY);
    }

    const std::string& RequestImpl::getScriptFilename() const {
        return Parser::get(vars_, SCRIPT_FILENAME_KEY);
    }

    const std::string& RequestImpl::getDocumentRoot() const {
        return Parser::get(vars_, DOCUMENT_ROOT_KEY);
    }

    const std::string& RequestImpl::getRemoteUser() const {
        return Parser::get(vars_, REMOTE_USER_KEY);
    }

    const std::string& RequestImpl::getRemoteAddr() const {
        return Parser::get(vars_, REMOTE_ADDR_KEY);
    }

    const std::string& RequestImpl::getQueryString() const {
        return Parser::get(vars_, QUERY_STRING_KEY);
    }

    const std::string& RequestImpl::getRequestMethod() const {
        return Parser::get(vars_, REQUEST_METHOD_KEY);
    }

    std::streamsize RequestImpl::getContentLength() const {
        const std::string& header = Parser::get(headers_, CONTENT_LENGTH_KEY);
        if (header.empty()) {
            return 0;
        }
        try {
            return str_cast_i<std::streamsize>(header);
        }
        catch (...) {
        }
        return 0;
    ...
    const std::string& RequestImpl::getContentType() const {
        return Parser::get(headers_, CONTENT_TYPE_KEY);
    }

    unsigned int RequestImpl::countArgs() const {
        return args_.size();
    }

    bool RequestImpl::hasArg(const std::string &name) const {
        for (auto i = args_.cbegin(), end = args_.cend(); i != end; ++i) {
            if (i->first == name) {
                return true;
            }
        }
    ...
    const std::string& RequestImpl::getArg(const std::string &name) const {
        for (auto i = args_.cbegin(), end = args_.cend(); i != end; ++i) {
            if (i->first == name) {
                return i->second;
            }
    ...
    void RequestImpl::getArg(const std::string &name, std::vector<std::string> &v) const {
    ...
    void RequestImpl::argNames(std::vector<std::string> &v) const {
    ...
    unsigned int RequestImpl::countHeaders() const {
    ...
    const std::string& RequestImpl::getHeader(const std::string &name) const {
        return Parser::get(headers_, name);
    }

    void RequestImpl::headerNames(std::vector<std::string> &v) const {
        Parser::keys(headers_, v);
    }

    unsigned int RequestImpl::countCookie() const {
    ...
    void
        ...
        for (auto i = files_.cbegin(), end = files_.cend(); i != end; ++i) {
    ...
    void
        ...
        if (!headers_sent_) {
            ...
            throw std::runtime_error("Error in RequestImpl::setStatus headers already sent: status - '" + i_cast_str<unsigned short>(status) + "'");
        }
    }
    ...
    void
        ...
        if (!headers_sent_) {
            ...
            throw std::runtime_error("Error in RequestImpl::setError headers already sent: status - '" + i_cast_str<unsigned short>(status) + "'");
        }
        status_ = status;
        ...
        if (stream_) {
            ...
            std::string status_str = i_cast_str<unsigned short>(status);
    ...
    void
        ...
        uint64_t size = getContentLength();
        if (cache_ && size >= cache_->minPostSize()) {
            post_buffer = cache_->create();
            uint64_t shift = serializeEnv(post_buffer, size + sizeof(uint64_t));
        ...
        if (!post_buffer.isNil()) {
            uint64_t pos = post_buffer.size();
            post_buffer.resize(pos + 2 * sizeof(uint64_t) +
    ...
    void
        ...
        if (!headers_sent_) {
            ...
            if (stream_) {
                for (auto i = out_headers_.begin(), end = out_headers_.end(); i != end; ++i) {
                    stream_->write(i->first.c_str(), i->first.size());
                    stream_->write(": ", 2);
                    stream_->write(i->second.c_str(), i->second.size());
                    stream_->write("\r\n", 2);
                }
                for (auto i = out_cookies_.cbegin(), end = out_cookies_.cend(); i != end; ++i) {
    ...
    uint64_t
        RequestImpl::serializeEnv(DataBuffer &buffer, uint64_t add_size) {
        uint64_t header_size = 0;
        for (auto it = headers_.begin(), end = headers_.end(); it != end; ++it) {
            header_size += it->first.size();
            header_size += it->second.size();
            header_size += 2 * sizeof(uint64_t);
        }
        uint64_t cookie_size = 0;
        for (auto it = cookies_.begin(), end = cookies_.end(); it != end; ++it) {
            cookie_size += it->first.size();
            cookie_size += it->second.size();
            cookie_size += 2 * sizeof(uint64_t);
        }
        uint64_t var_size = 0;
        for (auto it = vars_.begin(), end = vars_.end(); it != end; ++it) {
            var_size += it->first.size();
            var_size += it->second.size();
            var_size += 2 * sizeof(uint64_t);
        }
        buffer.resize(add_size + header_size + cookie_size +
            var_size + 3 * sizeof(uint64_t));
    ...
    uint64_t RequestImpl::serializeInt(DataBuffer &buffer, uint64_t pos, uint64_t val) {
        pos += buffer.write(pos, (char*)&val, sizeof(val));
        return pos;
    }

    uint64_t RequestImpl::serializeString(DataBuffer &buffer, uint64_t pos, const std::string &val) {
    ...
    void
        ...
        add_size += body_.size();
        add_size += sizeof(uint64_t);
        uint64_t file_size = filesSerializedSize();
        add_size += file_size;
        add_size += sizeof(uint64_t);
        uint64_t arg_size = argsSerializedSize();
        add_size += arg_size;
        uint64_t pos = serializeEnv(buffer, add_size);
    ...
    uint64_t
        RequestImpl::serializeFiles(DataBuffer &buffer, uint64_t pos) {
        uint64_t file_size = filesSerializedSize();
    ...
    uint64_t
        RequestImpl::serializeArgs(DataBuffer &buffer, uint64_t pos) {
        uint64_t arg_size = argsSerializedSize();
    ...
    uint64_t
        RequestImpl::parseInt(DataBuffer buffer, uint64_t pos,
            uint64_t &val) {
    ...
    uint64_t
        RequestImpl::parseString(DataBuffer buffer, uint64_t pos, std::string &val) {
        uint64_t len = 0;
    ...
    uint64_t
        RequestImpl::parseHeaders(DataBuffer buffer, uint64_t pos) {
        uint64_t field_size = 0;
        pos = parseInt(buffer, pos, field_size);
        uint64_t pos_end = pos + field_size;
    ...
    uint64_t
        RequestImpl::parseCookies(DataBuffer buffer, uint64_t pos) {
        uint64_t field_size = 0;
        pos = parseInt(buffer, pos, field_size);
        uint64_t pos_end = pos + field_size;
        if (pos_end > buffer.size()) {
    ...
    uint64_t
        RequestImpl::parseVars(DataBuffer buffer, uint64_t pos) {
        uint64_t field_size = 0;
        pos = parseInt(buffer, pos, field_size);
        uint64_t pos_end = pos + field_size;
        if (pos_end > buffer.size()) {
    ...
    uint64_t
        RequestImpl::parseBody(DataBuffer buffer, uint64_t pos) {
        uint64_t field_size = 0;
        pos = parseInt(buffer, pos, field_size);
        uint64_t pos_end = pos + field_size;
        if (pos_end > buffer.size()) {
    ...
    uint64_t
        RequestImpl::parseFiles(DataBuffer buffer, uint64_t pos) {
        uint64_t field_size = 0;
        pos = parseInt(buffer, pos, field_size);
        uint64_t pos_end = pos + field_size;
        if (pos_end > buffer.size()) {
    ...
    uint64_t
        RequestImpl::parseArgs(DataBuffer buffer, uint64_t pos) {
        uint64_t field_size = 0;
        pos = parseInt(buffer, pos, field_size);
        uint64_t pos_end = pos + field_size;
        if (pos_end > buffer.size()) {
    ...
    void
        ...
        uint64_t pos = parseHeaders(buffer, 0);
    ...
    unsigned short RequestImpl::status() const {
        return status_;
    }
