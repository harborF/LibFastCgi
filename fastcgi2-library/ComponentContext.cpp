...
namespace fastcgi
{
    ...
    const Config* ComponentContextImpl::getConfig() const {
        return globals_->config();
    }

    const Globals* ComponentContextImpl::globals() const {
        return globals_;
    }

    std::string ComponentContextImpl::getComponentXPath() const {
        return componentXPath_;
    }

    Component* ComponentContextImpl::findComponentInternal(const std::string &name) const {
