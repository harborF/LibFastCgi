...
namespace fastcgi
{
    ...
    void Request::headerNames(std::vector<std::string> &v) const {
        impl_->headerNames(v);
    }

    unsigned int Request::countCookie() const {
    ...
    void Request::cookieNames(std::vector<std::string> &v) const {
        impl_->cookieNames(v);
    }
    ...
    void Request::remoteFiles(std::vector<std::string> &v) const {
        impl_->remoteFiles(v);
    }

    const std::string& Request::remoteFileName(const std::string &name) const {
        return impl_->remoteFileName(name);
    }

    const std::string& Request::remoteFileType(const std::string &name) const {
        return impl_->remoteFileType(name);
    }

    DataBuffer Request::remoteFile(const std::string &name) const {
    ...
    DataBuffer Request::requestBody() const {
    ...
    void Request::setStatus(unsigned short status) {
        impl_->setStatus(status);
    }

    void Request::sendError(unsigned short status) {
    ...
    void Request::write(std::streambuf *buf) {
        impl_->write(buf);
    }

    std::streamsize Request::write(const char *buf, std::streamsize size) {
        return impl_->write(buf, size);
    }

    std::string Request::outputHeader(const std::string &name) const {
    ...
    void Request::attach(RequestIOStream *stream, char *env[]) {
    ...
    void Request::markAsProcessed() {
        impl_->markAsProcessed();
    }

    void Request::tryAgain(time_t delay) {
        impl_->tryAgain(delay);
    }

    void Request::parse(DataBuffer buffer) {
        impl_->parse(buffer);
    }

    void Request::serialize(DataBuffer &buffer) {
        impl_->serialize(buffer);
    }

    void Request::redirectBack() {
        redirectToPath(getHeader("Referer"));
    }

    void Request::redirectToPath(const std::string &path) {
        setStatus(302);
        setHeader("Location", path);
    }

    void Request::setContentType(const std::string &type) {
        setHeader("Content-type", type);
    }

    void Request::setContentEncoding(const std::string &encoding) {
        setHeader("Content-encoding", encoding);
    }

    unsigned short Request::status() const {
        return impl_->status();
    }
