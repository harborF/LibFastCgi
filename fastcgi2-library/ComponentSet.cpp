...
namespace fastcgi
{
    ...
    void ComponentSet::init(const Globals *globals) {
        globals_ = globals;
    ...
    Component* ComponentSet::find(const std::string &name) const {
    ...
    void ComponentSet::add(const std::string &name, const std::string &type, const std::string &componentXPath) {
    ...
    void ComponentSet::sendOnLoadToComponents() {
        for (ComponentMap::iterator i = components_.begin(); i != components_.end(); ++i) {
            sendOnLoad(i->first, i->second);
        }
    }

    void ComponentSet::sendOnLoad(const std::string &componentName, ComponentContainer &cont) {
    ...
    void ComponentSet::sendOnUnloadToComponents() {
    ...
    bool ComponentSet::isComponentLoaded(const std::string &componentName) const {
