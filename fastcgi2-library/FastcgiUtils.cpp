...
namespace fastcgi
{
    ...
    std::string StringUtils::urlencode(const Range &range) {
    ...
    void StringUtils::urldecode(const Range &range, std::string &result) {
        for (const char *i = range.begin(), *end = range.end(); i != end; ++i) {
            switch (*i) {
            case '+':
                result.append(1, ' ');
    ...
    std::string StringUtils::urldecode(const Range &range) {
        std::string result;
    ...
    std::string StringUtils::urldecode(DataBuffer data) {
        std::string result;
    ...
    void StringUtils::parse(const Range &range, std::vector<NamedValue> &v) {
    ...
    void StringUtils::parse(DataBuffer data, std::vector<NamedValue> &v) {
    ...
    void StringUtils::parse(const std::string &str, std::vector<NamedValue> &v) {
    ...
    std::string HttpDateUtils::format(time_t value) {
    ...
    time_t HttpDateUtils::parse(const char *value) {
    ...
    std::string HashUtils::hexMD5(const char *key, unsigned long len) {
