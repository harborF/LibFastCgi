...
namespace fastcgi
{
    static const time_t MAX_EXPIRES_TIME = std::numeric_limits<int32_t>::max();
    ...
    void Cookie::expires(time_t value) {
        data_->expires(value);
    }
    ...
    void Cookie::path(const std::string &value) {
        data_->path(value);
    }
    ...
    void Cookie::domain(const std::string &value) {
        data_->domain(value);
    }

    bool Cookie::permanent() const {
        return data_->permanent();
    }

    void Cookie::permanent(bool value) {
    ...
    void Cookie::urlEncode(bool value) {
