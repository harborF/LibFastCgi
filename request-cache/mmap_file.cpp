...
namespace fastcgi
{
    ...
    MMapFile::MMapFile(const char *name, uint64_t window, bool is_read_only) :
    ...
    void
        ...
        if (0 == window_) {
            ...
            uint64_t window_index = window_ / page_size_;
            uint64_t offset = window_ - window_index * page_size_;
            if (0 != offset) {
                ++window_index;
            }
    ...
    void
        MMapFile::resize(uint64_t newsize) {
        unmap();
    ...
    char
        MMapFile::at(uint64_t index) {
        check_index(index);
        if (mapped(index)) {
            return *((char*)pointer_ + (index - segment_start_));
        }
        uint64_t offset = index % window_, segment = index / window_;
        return *(map_segment(segment) + offset);
    }

    char*
        MMapFile::atRange(uint64_t index, uint64_t length) {
        check_index(index);
        if (length > 0) {
    ...
    std::pair<char*, uint64_t>
        MMapFile::atSegment(uint64_t index) {
        check_index(index);
        uint64_t segment_pos = (index / window_) * window_;
        uint64_t len = std::min(segment_pos + window_, size_) - index;
    ...
    char* MMapFile::map_segment(uint64_t segment) {
        uint64_t pos = segment * window_;
        return map_range(pos, pos + window_);
    }

    char* MMapFile::map_range(uint64_t begin, uint64_t end) {
        if (mapped(begin, end)) {
            return (char*)pointer_ + begin - segment_start_;
        }
        unmap();
        uint64_t begin_base = page_size_ * (begin / page_size_);
        if (is_read_only_) {
    ...
    void MMapFile::check_index(uint64_t index) const {
        if (index >= size_) {
            throw std::out_of_range("mapped file index out of range");
        }
    }

    uint64_t MMapFile::window() const {
        return window_;
    }

    bool MMapFile::mapped(uint64_t index) const {
        return mapped(index, index + 1);
    }

    bool
        MMapFile::mapped(uint64_t begin, uint64_t end) const {
